...
export default function PrikazPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <h1 className="text-3xl font-bold mb-6">Proizvodi</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {products.map((product) => (
          <Link key={product.id} href={`/detalji/${product.id}`}>
            <div className="bg-gray-800 p-4 rounded-xl shadow-lg hover:bg-gray-700 cursor-pointer">
              <img
        ...
        {products.map((product) => (
          <Link key={product.id} href={`/detalji/${product.id}`}>
            <div className="bg-gray-800 p-4 rounded-xl shadow-lg hover:bg-gray-700 cursor-pointer">
              ...
              />
              <h2 className="text-xl font-semibold">{product.title}</h2>
              <p className="text-gray-300 mt-2 text-sm overflow-hidden text-ellipsis"
                 style={{
                   display: '-webkit-box',
                   WebkitLineClamp: 2,
...
export default function PoklanjamPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch('/api/poklanjam');  // Promenjena ruta
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || 'Greška pri dohvatu proizvoda');
        setProducts(data);
      } catch (err) {
