...
export async function POST(req) {
  try {
    ...
    const image = formData.get('image');
    const userId = formData.get('userId');  // Dodano: uzimanje userId sa forme

    console.log('📦 Podaci iz forme:', { title, description, grad, image, userId });
    ...
    if (!userId) {
      return NextResponse.json({ error: 'Nema korisničkog ID-a' }, { status: 400 });
    }
      ...
      .insert([
        {
          ...
          grad,
          image_url: imageUrl,
          user_id: userId,  // Dodano: spremanje userId u bazu
        },
      ]);
